...
class ModelV1(BaseClassifier):
  ...
  def FPropTower(self, theta, input_batch):
    ...
    # FC
    act = self.fc1.FProp(theta.fc1, tf.reshape(act, [batch, -1]))

    act = self.fc2.FProp(theta.fc2,act)
...
class GPipeModelVGG16(BaseClassifier):
    ...
    def Params(cls):
      p = super().Params()
      p.Define('im',im_layers.GPipeImVGG16.CommonParams(),'the layers of the image model you want')
      tp = p.train
      tp.learning_rate = 1e-4  # Adam base LR.
      tp.lr_schedule = (
          schedule.LinearRampupExponentialDecayScaledByNumSplitSchedule.Params()
